...
namespace Calyx {
	...
	void Framebuffer::Invalidate()
	{
		...
		{
			CX_CORE_ASSERT(m_ColorAttachments.size() <= 4);
		...
		CX_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void Framebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			CX_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	{
		CX_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
