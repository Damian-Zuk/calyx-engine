...
namespace Calyx {
	...
	void Logger::Init()
	{
		...
		logSinks.emplace_back(MakeShared<spdlog::sinks::basic_file_sink_mt>("Calyx.log", true));

		logSinks[0]->set_pattern("%^[%T] [%l] %n: %v%$");
		logSinks[1]->set_pattern("[%T] [%l] %n: %v");

		s_CoreLogger = MakeShared<spdlog::logger>("Calyx", begin(logSinks), end(logSinks));
	...
	std::string LoggerUtils::FormatFuncSignature(const std::string& funcsig)
	{
		...
		if (funcsig.find("Calyx::", pos) != std::string::npos)
			pos += 8;
